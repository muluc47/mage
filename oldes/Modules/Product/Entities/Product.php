...
class Product extends Model
{
    ...
    protected static function boot()
    {
        ...
        static::saved(function ($product) {
            ...
            $product->withoutEvents(function () use ($product) {
                $product->update(['selling_price' => $product->getSellingPrice()]);
            });
        });
    ...
    public function hasStockFor($qty)
    {
        ...
        $addedQty = Cart::addedQty($this->id);

        return ($this->qty - $addedQty) >= $qty;
    }
    ...
    public function getSellingPrice()
    {
        if ($this->hasSpecialPrice()) {
            return $this->getSpecialPrice()->amount();
        }
    ...
    {
        $specialPrice = $this->getOriginal('special_price');

        if ($this->special_price_type === 'percent') {
            $discountedPrice = ($specialPrice / 100) * $this->getOriginal('price');

            $specialPrice = $this->getOriginal('price') - $discountedPrice;
        }
