...
use Modules\Core\Foundation\Asset\Manager\BaslatAssetManager;
use Modules\Core\Foundation\Asset\Pipeline\BaslatAssetPipeline;
...
class AssetServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(AssetManager::class, BaslatAssetManager::class);

        $this->app->singleton(AssetPipeline::class, function ($app) {
            return new BaslatAssetPipeline($app[AssetManager::class]);
        });
    }
    ...
    private function addThemesAssets()
    {
        ...
        if (! is_null($assets = config("baslat.theme.{$theme}.assets"))) {
    ...
    private function addModulesAssets()
    {
        foreach ($this->app['modules']->allEnabled() as $module) {
            $assets = config("baslat.module.{$module->getLowerName()}.assets");
