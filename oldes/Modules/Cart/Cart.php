...
class Cart extends DarryldecodeCart
{
    ...
    private function options($product, $options)
    {
        ...
        return $this->mergeFiledOptions($productOptions, $options);
    }

    private function mergeFiledOptions($productOptions, $options)
    {
        $filteredOptions = collect($options)->reject(function ($value, $optionId) use ($productOptions) {
            return $productOptions->contains('id', $optionId);
        });

        $fieldOptions = Option::with('values')
            ->whereIn('id', $filteredOptions->keys())
            ->get();

        return $filteredOptions->map(function ($value, $optionId) use ($fieldOptions) {
            return optional($fieldOptions->where('id', $optionId)->first(), function ($option) use ($value) {
                $option->values
                    ->first()
                    ->fill(['label' => $value]);
