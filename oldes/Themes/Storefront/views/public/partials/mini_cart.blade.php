                    ...
                    @foreach ($cart->items() as $cartItem)
                        <div class="mini-cart-item clearfix">
                            <div class="mini-cart-image">
                                ...
                                @else
                                    <img src="{{ $cartItem->product->base_image->path }}" alt="{{ $cartItem->product->name }}">
                                @endif
                            </div>
                                ...
                                @if ($cartItem->options->isNotEmpty())
                                    <div class="mini-cart-variants dropdown pull-left">
                                        <div class="dropdown-toggle" data-toggle="dropdown">
                                            {{ $cartItem->options->first()->name }}: <span>{{ str_limit($cartItem->options->first()->values->implode('label', ', '), 10) }}</span>
                                        ...
                                        @if ($cartItem->options->count() > 1)
                                            <ul class="dropdown-menu">
                                                @foreach ($cartItem->options->slice(1) as $option)
                                                    <li>
                                                        {{ $option->name }}: <span>{{ str_limit($option->values->implode('label', ', '), 10) }}</span>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        @endif
